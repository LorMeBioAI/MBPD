...
cd {PWD}01.ASV.tax/
####blast######
makeblastdb -in {abs_db}/pathogen.fasta -dbtype nucl -parse_seqids -out {abs_db}/MBPD
...
perl {abs_db}/add.m8.table.pl asv_table.xls m8.txt
...
with open(mode.format("blast_analysis") + '.r', 'w', encoding='utf-8') as r_file:
    r_content = f"""
library(dplyr)
database <- read.delim("pathogen.tax", header = FALSE)
temp <- read.delim("asv_taxa_table.xls")
asv <- left_join(temp, database, by = c("OTU.ID" = "V1"))
